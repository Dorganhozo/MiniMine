...
public class PerlinNoise2D {
    ...
    public static float grad(int hash, float x, float y) {
        int h = hash & 7;
        float[] g = GRADIENTES[h];
        return g[0] * x + g[1] * y;
    }
	
	public static float ruidoFractal2D(float x, float z, float escala, int seed, int octaves, float persis) {
		float total = 0;
		float amplitude = 1;
		float maxValor = 0;

		for(int i = 0; i < octaves; i++) {
			total += PerlinNoise2D.ruido(x * escala, z * escala, seed + i) * amplitude;
			maxValor += amplitude;
			amplitude *= persis;
			escala *= 2;
		}
